...
/* Static mode. The GitHub Pages copy sets window.SANAD_STATIC, because Pages serves
   files and this app has a Python API behind it. In that mode every read comes from a
   committed snapshot of the real API taken on the date in the banner, the audit works
   in full because it is a read, and the two routes that would touch a chain from the
   browser say so instead of pretending. Run the server for the live version. */
const STATIC = typeof window !== "undefined" ? window.SANAD_STATIC || null : null;
...
async function loadConfig() {
  CONFIG = await (await fetch(apiUrl("config"))).json();
...
/* The same lookup the API does, run over the snapshot the static copy already loaded.
   The server route filters on the indexed memo id; here the runs are already in memory,
   so it is the same answer without a second round trip. */
function staticReferenceLookup(reference) {
  if (!LEDGER) return { found: false, reference };
  for (const run of LEDGER.runs || []) {
    for (const payment of run.payments || []) {
      if (payment.reference === reference) return { found: true, runId: run.runId, payment };
    }
  }
...
async function post(path, body) {
  if (STATIC) {
    if (path.endsWith("/preview")) return await (await fetch(apiUrl("preview"))).json();
    throw new Error(
      "This is the static copy on GitHub Pages, so it cannot settle. Clone the repo and " +
      "run uvicorn sanad.api.app:app --app-dir src --port 8099 to settle on Arc.");
  }
...
async function rebuild() {
  ...
  const data = await (await fetch(apiUrl("ledger"))).json();
  LEDGER = data;
...
$("#find").addEventListener("click", async () => {
  ...
  const data = STATIC
    ? staticReferenceLookup(reference)
    : await (await fetch(`/api/ledger/reference/${encodeURIComponent(reference)}`)).json();
  if (!data.found) {
